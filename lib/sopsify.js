	...
	for (const clusterItem of clusterConfigs) {
		...
		for (const templateDef of templateDefs) {
			...
			for (const v of Object.values(section)) {
				if (typeof v === 'string') {
					const matches = v.match(placeholderRegex);
					if (matches) {
						for (const match of matches) {
							const placeholderName = getPlaceholderName(match);
							if (placeholderName) {
			...
			for (const ns of allNamespaces) {
				...
				const placeholderReplaceRegex = /\$\{(?<name>[a-zA-Z_][a-zA-Z0-9_-]*)\}/g;

				for (const [k, v] of Object.entries(section)) {
					if (typeof v === 'string') {
						section[k] = v.replace(placeholderReplaceRegex, (match, name) => {
							usedKeys.add(name);
							if (keyNamespaceValueMap[name] && keyNamespaceValueMap[name][ns] !== undefined) {
								return keyNamespaceValueMap[name][ns];
							}
							return match; // Should not happen in practice if validation is correct
						});
					}
				}
